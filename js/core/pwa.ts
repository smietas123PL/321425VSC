...
// M-07: Use static /manifest.json instead of a Blob URL.
// Blob URLs are blocked by iOS Safari for PWA installation and can trigger
// CSP violations. The static manifest.json is already present in the build
// root and is correcly served with application/manifest+json by Vite/Netlify.
const manifestLink = document.getElementById('pwa-manifest') as HTMLLinkElement | null;
if (manifestLink && !manifestLink.href.includes('manifest.json')) {
