(function () {
  ...
  async function generateShareUrl() {
    ...
    try {
      ...
      const displayEl = (document.getElementById('share-url-display') as HTMLInputElement);
      if (displayEl) displayEl.value = shareUrl;

      const kb = (shareUrl.length / 1024).toFixed(1);
      const sizeEl = (document.getElementById('share-size-label') as HTMLElement);
      if (sizeEl) {
        sizeEl.textContent = `${kb} KB`;
        sizeEl.className = parseFloat(kb) > 100 ? 'share-size-warn' : '';
      }

      const agentEl = (document.getElementById('share-agent-count') as HTMLElement);
      if (agentEl) agentEl.textContent = `${generatedAgents.length} ${tr('agents', 'agentow')}`;
      const verEl = (document.getElementById('share-version-label') as HTMLElement);
      ...
      trackEvent('share_created', {
        success: false,
        reason: String((e as any)?.message || 'share_error').slice(0, 120),
      });
    }
  }
  ...
  function onShareModeChange() {
    ...
    const pwRow = (document.getElementById('share-password-row') as HTMLElement);
  ...
  function openShareModal() {
    ...
    const openOpt = (document.querySelector('input[name="share-mode"][value="open"]') as HTMLInputElement);
    ...
    const gistResult = (document.getElementById('gist-result') as HTMLElement);
    if (gistResult) gistResult.style.display = 'none';
    const gistLabel = (document.getElementById('gist-publish-label') as HTMLElement);
    if (gistLabel) gistLabel.textContent = tr('⬆ Publish Gist', '⬆ Publikuj Gist');
    const gistBtn = (document.getElementById('gist-publish-btn') as HTMLButtonElement);
  ...
  async function copyShareUrl() {
    ...
    try {
      ...
      const btn = (document.getElementById('share-copy-btn') as HTMLElement);
      if (btn) {
