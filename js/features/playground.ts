...
(function () {
  ...
  function renderPlaygroundTabs(): void {
    ...
    container.innerHTML = '';
    state.generatedAgents.forEach((agent: any, i: number) => {
  ...
  function exportPlaygroundChat(): void {
    ...
    md += `**Project:** ${state.currentTopic || 'AgentSpark'}  \n`;
    md += `**Agent:** ${agentName}  \n`;
    ...
    const slug = (state.currentTopic || 'chat').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
