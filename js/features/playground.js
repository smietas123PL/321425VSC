(function () {
  ...
  function selectPlaygroundAgent(agent, btnEl) {
  ...
  function pgAddMessage(role, text) {
    const msgs = document.getElementById('playground-messages');
  ...
  async function sendPlaygroundMessage() {
    ...
    document.getElementById('playground-status').textContent = `${playgroundAgent.name} is thinking…`;
    ...
    try {
      ...
      document.getElementById('playground-status').textContent = `${playgroundHistory.filter((m) => m.role === 'assistant').length} responses`;
    } catch (e) {
      if (typingDiv.parentNode) msgs.removeChild(typingDiv);
