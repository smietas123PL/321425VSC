...
let _galleryFiltered = [...GALLERY_TEMPLATES];
...
function filterByCategory(btnEl, cat) {
...
function _cleanStr(val, maxLen = 180) {
  return String(val || '').replace(/\s+/g, ' ').trim().slice(0, maxLen);
}

function _cleanList(arr, maxItems = 12, maxLen = 120) {
...
function normalizeTemplate(raw) {
  ...
  const team = raw.team.map(member => {
...
function showTemplateDetail(id) {
  ...
  document.getElementById('tpl-modal-title').textContent = t.title;
  document.getElementById('tpl-badge').textContent = t.featured ? tr('Featured', 'Polecany') : t.category;
  document.getElementById('tpl-badge').className = `template-badge ${t.featured ? 'featured' : ''}`;
  document.getElementById('tpl-difficulty').textContent = `📊 ${t.difficulty}`;
  document.getElementById('tpl-agent-count').textContent = `🤖 ${t.agentCount} ${tr('agents', 'agentow')}`;
  document.getElementById('tpl-description').textContent = t.description;
  const useCasesLabel = document.getElementById('tpl-usecases-label');
  const teamLabel = document.getElementById('tpl-team-label');
  const forkBtn = document.getElementById('tpl-fork-btn');
  ...
  const useCases = document.getElementById('tpl-usecases');
  const team = document.getElementById('tpl-team');
  ...
  document.getElementById('template-detail-overlay').classList.add('open');
}

function closeTemplateDetail() {
  document.getElementById('template-detail-overlay').classList.remove('open');
...
function forkTemplateById(id) {
  ...
  const fallback = ['🤖','🧠','⚙️','📊','🎯','💡'];
  const emojis = catEmojis[t.category] || fallback;
